...
function updateMoveValue(moveInput, moveInputId) {
  ...
  if (!updatedValue || updatedValue < 0) {
    updatedValue = 1;
    $('#' + moveInputId).val(1);
  } else if ((unchangedValueSum + updatedValue) > 33) {
    updatedValue = 33 - unchangedValueSum;
    $('#' + moveInputId).val(updatedValue);
  }
  $('#remaining-strength').text(33 - unchangedValueSum - updatedValue);
}
